...
class ObjectSortPaginate:
    def get_pagination(self, feeds_all, obj_selected_category=None, count=4):
        # query_search_feed = self.request.GET.get('search-feed', '')
        # paginator = Paginator(feeds_all.filter(Q(title__icontains=query_search_feed)|
        #                                        Q(body__icontains=query_search_feed)|
        #                                        Q(tag__title__icontains=query_search_feed)), count)

        paginator = Paginator(feeds_all, count)
